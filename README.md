# Reinforced Storage 26.2 Compatibility Port (Build 13)
...
This is a community port, not an official Aton-Kish release or endorsement. The Build 13
...
## Build 13 standard-layout GUI compatibility

Build 13 keeps the validated migration behavior, placed-chest renderer and Nemo menu marker.
It includes Build 12's complete-cell renderer and fixes the remaining wide-layout Nemo button alignment:
...
Nemo's default lower buttons are initially anchored to the full container `imageWidth`. On wider
storage screens, the client screen moves only those four default lower widgets left by the same
expansion used to center the player panel. The upper container buttons continue to follow the
container edge. Explicit Nemo x/y configuration remains authoritative.
...
See `docs/BUILD12_RENDERER_FIX.md` and `docs/BUILD13_RENDERER_FIX.md` for the exact contract and
validation limits.

Run `bash tools/build_barrel_chest.sh` with Java 25 and Gradle to compile. The build now executes the
production Java screen in a recording harness and checks cell coverage, frame placement, intact
player artwork, absence of shoulder filler and Nemo lower-row alignment. These are executable
composition tests, not proof of Minecraft/Fabric linkage or runtime behavior.
